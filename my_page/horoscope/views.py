...
def get_zodiac_info_by_number(request, zodiac_sign: int):
    ...
    name_zodiac = zodiacs[zodiac_sign - 1]  # Если все хорошо, то мы преобразуем полученное число в знак зодиака
    # и по полученному URL обратимся к нужной нам функцииfrom django.http import HttpResponse, HttpResponseNotFound, HttpResponseRedirect
...
def get_zodiac_info_by_number(request, zodiac_sign: int):
    zodiacs = list(zodiac_dict)  # Присваиваем переменной список из ключей словаря
    if zodiac_sign > len(zodiacs) or zodiac_sign == 0:  # Если введенное пользователем число будет больше
        # длины нашего списка, вернется соответствующее сообщение
        return HttpResponseNotFound(f'Неправильный порядковый номер знака зодиака - {zodiac_sign}!')
    name_zodiac = zodiacs[
        zodiac_sign - 1]  # Если все хорошо, то мы преобразуем полученное число в знак зодиака
    # и по полученному URL обратимся к нужной нам функции
